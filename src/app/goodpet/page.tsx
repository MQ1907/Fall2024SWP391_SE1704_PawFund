...
const GoodBaby = () => {
  ...
  return (
    ...
    <div className={`flex flex-col items-center justify-center pt-10 gap-3 ${isAnimating ? 'animate__animated animate__fadeInLeft animate__delay-3s animate__duration-4s' : ''}`}>
      <div className="font-semibold text-3xl text-gray-800">GOOD BABY OF THE WEEK</div>
    ...
    <div className="w-full max-w-6xl bg-white mx-auto shadow-lg rounded-lg p-8 relative mt-5">
      <div className="flex justify-between items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {currentPets.map((pet, index) => (
            <div key={pet._id} className={`relative text-center mx-auto animate__animated ${pet.isAdopted ? 'bg-gray-300' : 'bg-white'} shadow-md rounded-lg p-4 transition duration-300 ease-in-out hover:shadow-lg`}>
              ...
              <h3 className="text-lg font-bold mt-4 text-gray-800">{pet.name}</h3>
              ...
              <p className="text-gray-600 font-semibold">Adopted: {pet.isAdopted ? 'Already adopted' : 'Not yet'}</p>
            </div>
          ))}
        </div>
      ...
      <div className="flex justify-center mt-8">
        <button onClick={() => { router.push('/adopt') }} className="bg-pink-600 text-white py-3 px-20 rounded-full font-semibold hover:bg-[#018AE0] mt-10 animate__animated animate__pulse animate__infinite transition duration-300 ease-in-out">
          ADOPT
        </button>
      </div>
    </div>
